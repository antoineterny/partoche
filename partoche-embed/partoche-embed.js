...
window.onload = () => {
    ...
    partocheInstances.forEach(partocheDiv => {
        ...
        if (partocheDiv.getAttribute('data-type') === "audio") {
            ...
            playBtn.addEventListener("click", togglePlayPause.bind(newAudioPlayer))
            ...
            titre.addEventListener("click", (event) => {
                ...
                const playBtn = event.target.parentNode.parentNode.querySelector('.playBtn')
...
function previousRegion(part) {
    const player = document.querySelector(`#${part.id} audio, #${part.id} video`)
    ...
    for (let i=1; i<=part.pageOffsets.length; i++) {
      ...
      newImg.onload = function() {
        ...
        if (part.pagesLoaded == part.pageOffsets.length) {
            part.pagesHeight = Array
                .from(partitionDiv.querySelectorAll(".pages img"))
                .map((x) => parseFloat(getComputedStyle(x).height));
            // for (let i=0; i<part.pagesHeight.length; i++) {
            //     part.previousPagesHeight.push(getTotalPreviousHeight(i+1));
            // }
        }
      }
    ...
    allPartoches.forEach(part => {
        ...
        for (let i = 0; i < part.regions.length; i++) {
            if (
                ...
                part.pageOffsets[part.regions[i].page - 1][part.regions[i].line - 1];
